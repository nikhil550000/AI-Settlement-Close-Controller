...
| | |
|---|---|
| [What this is](#what-this-is) · [Quickstart](#quickstart--reproduce-path) · [Where the model may act](#where-the-model-is-allowed-to-act-exactly) | start here |
| [Result 1: vocabulary](#result-1-where-the-model-earns-its-place-and-where-it-does-not) · [Result 2: judgment](#result-2-the-model-returns-rs-7323-of-bank-credit-and-the-gate-that-makes-it-safe) · [Bank-line accounting](#bank-line-accounting-rs-1269320-went-missing-and-no-test-could-see-it) · [Metrics](#metrics-and-evaluation) | the evidence |
| [Bank-profile inference](#the-agentic-surface-bank-profile-inference) · [The shape of the system](#the-shape-of-the-system) · [How one case reaches its state](#how-one-case-reaches-its-state) · [Datasets](#batch-and-datasets) | how it is built |
| [Glossary](#glossary) · [Non-goals](#non-goals) · [Required disclosures](#required-disclosures) · [Known limitations](#known-limitations) · [What broke](#what-broke-and-how-it-was-recovered) | the caveats |
...
|---|---|---|---|
| **Semantics** — five of the six free-text reads (the sixth, the contested-credit read, is its own row below) | `pipeline/semantics.py` | does this narration name a counterparty; is this credit the gateway; is this a reversal; is this a bank charge; is this a tax position | yes — the whole §1.6 surface, both arms |
...
## Result 2: the model returns Rs 7,323 of bank credit, and the gate that makes it safe
...
**The result, before the story.** On twelve hand-authored cases built around FR-09 tier-2 contention, the grounded model read attaches **Rs 7,323.00 of real bank credit** to the settlement that owns it, money the deterministic arm leaves attached to nothing. It costs **zero false matches**, and the credits it still cannot place are a strict subset of the ones the keyword arm cannot place. Before the grounding gate existed, the same read answered a settlement id for a narration that named nothing: a coin flip presented as an answer, on a read where a coin flip books real money against the wrong settlement. The gate is what turns the first fact into something safe enough to ship.

How it got there is the part worth reading.
...
**The fourth column is why the headline above is denominated in rupees.** Two cases is a thin number to hang a claim on; Rs 7,323.00 moving from *attached to nothing* to *attached to the settlement that owns it* is not. `tests/test_bank_accounting.py` asserts the figure, the subset relation, and that resolving a contest moves money between dispositions without creating or destroying any.
...
## Bank-line accounting: Rs 12,693.20 went missing, and no test could see it
...
**Performance (FR-02, NFR-02, NFR-03), on `Windows AMD64, Intel64 Family 6 Model 154 Stepping 4, Python 3.11.15`, six runs each across two sessions:** reference batch (150 cases, 7,111 raw records) 598–795 cases/s; scale batch (seed 3, 362 cases, 19,355 raw records) 346–383 cases/s. These are the full observed spread across runs, not a best-of band: run-to-run variance on this machine is wider than the difference the `semantics` refactor made, so the refactor's cost is reported as "below the noise floor" rather than as a measured improvement. A quiet machine sits near the top of each range and a busy one near the bottom. Reproduce with `uv run python tools/measure_performance.py`.
...
Everything below is used above without ceremony. Nothing here needs `spec.md`.

| term | what it means |
|---|---|
| **case** | One unit of reconciliation work. A *settlement-anchored* case is every recon line sharing a `settlement_id`; an *orphan* case is a residual bank line that belongs to no settlement. The batch is 125 of the first and 25 of the second. |
| **terminal state** | Every case ends in exactly one of five: `AUTO_MATCHED` (clean, no entry needed), `AUTO_CLOSED` (a correcting entry was derived, validated, posted and re-reconciled to zero), `EXTERNAL_ACTION_REQUIRED` (categorized, but needs someone outside the system), `REVIEW_REQUIRED` (a human decides), `ABSTAINED` (evidence does not justify an automated decision). |
| **residual** | The unexplained difference on a case, in integer paise. Zero residual is what "reconciled" means here. Never a float. |
| **paise** | The only money unit in the pipeline. 1 rupee = 100 paise, stored as `int`. There is no floating-point arithmetic anywhere on the money path. |
| **tier 0–3** | The matcher's four-step cascade, strongest evidence first: **0** exact UTR, **1** UTR prefix, **2** exact amount inside the T+2 window, **3** no match. First hit wins. Tier 2's key is not unique to a settlement, which is what Result 2 is about. |
| **T+2 window** | A settlement is expected to reach the bank within two working days, weekends excluded, no holiday calendar. Past that with no credit, the case is overdue rather than merely pending. |
| **the six templates** | The entire allowlist of journal entries this system may post. `T-01` unposted gateway fee + GST where revenue was booked gross · `T-02` unposted refund · `T-03` the same fee + GST where revenue was booked net, so revenue is understated · `T-04` a premature bank debit with no matching credit · `T-05` an unposted settlement credit adjustment · `T-06` an unposted settlement debit adjustment. No seventh entry can be posted, by construction. |
| **Slot A** | The model surface that assigns an exception *subtype* to a case that will not auto-close. Graded. Measures worse than the deterministic baseline, and is not the default. |
| **Slot B** | The model surface that writes prose for a human reviewer. Off the money path, ungraded, labelled model-generated in the report. |
| **semantics** | The six free-text reads over bank and adjustment prose, behind one interface with a keyword arm and an LLM arm. Five route cases; the sixth is the contested-credit read on the money path. |
| **§x.y, FR-nn, NFR-nn, REV-nn** | Section, functional requirement, non-functional requirement and numbered revision in `spec.md`. They are there so a specific claim can be traced. `spec.md` is 1,251 lines and is not meant to be read end to end. |
...
<details>
<summary><b>8 more, in full</b></summary>
