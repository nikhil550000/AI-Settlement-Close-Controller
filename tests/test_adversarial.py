"""The adversarial-set checkpoint:

> Adversarial set runs and is reported separately.

The adversarial set's second mitigation: "A hand-authored adversarial set of 10-12 cases.
...
gitignored — the JSONL is the committed artifact, per the secrets rule) are hand-picked
literal records, not generator output, targeting the four boundaries named
in the adversarial set: `T-01` versus `T-03`, family 4 core versus its date-error
variant, duplicate credit versus reversal, and two "genuinely
...
components 2-8, the same deterministic keyword baseline)
...
is called on this batch alone, producing its own
...
the recorded figures come from this test's own run, not from
...
# The two noise lines (a self-matching reversal pair, per a revision's "matched
...
def test_the_self_matching_reversal_pair_raises_no_case_at_all(batch) -> None:
    """a revision's other half of the duplicate/reversal boundary: a reversal whose
...
def test_no_false_auto_match_or_wrong_auto_applied_entry(batch, result) -> None:
    ...
    Both are the primary safety metrics, asserted directly rather than only
...
def test_the_four_named_boundaries_resolve_to_the_hand_written_label(batch, result) -> None:
    """The adversarial set names four boundaries by hand; this pins the deterministic path's
...
def test_report_renders_and_is_never_folded_into_another_metrics_report(batch, result) -> None:
    ...
    (the dev-versus-held-out machinery) or merged with any other
    `MetricsReport`."""
    report = build_eval_report(result.cases, result.outcome.outcomes, batch["ground_truth"], arm="adversarial")
