"""The checkpoint: every denominator in the metrics report is hand-checked against
the batch's known totals.

The checkpoint test is `test_every_denominator_hand_checks_against_the_batch_totals_table`.
It transcribes the batch-totals table and the per-population allocations as
literals — typed out by hand, never read back from the generator — and
asserts every denominator in `MetricsReport` against them one by one. A
denominator computed from the same code that produced the numbers it divides
would be asserting nothing; the same is true of labels, and it applies with
more force to the figures those labels are divided by.

`test_denominator_convention_holds_for_every_metric` is the second half of
the hand-check: the convention that `*_rate` metrics are measured over total
cases, `*_recall` over the ground-truth population, and `*_precision` over
the predicted population, asserted as a property of every named metric, so a
future metric added with the wrong denominator fails here rather than in a
report.

Around those: unit coverage of `Rate`'s undefined case, the macro average's
seven-subtype denominator, `auto_applied_entries`' inclusion of replayed
entries, `count_matching_entries`' multiset semantics, `case_value_paise`'s
two anchor kinds, `_aligned`'s batch-mismatch guards, and determinism of the
whole report across two runs.

Everything is offline. The classifier used throughout is the deterministic
keyword baseline, so no test here needs a network, a key, or a cache file —
the same discipline `tests/test_llm_slot_a.py` and `tests/test_llm_slot_b.py`
follow.
"""
...
# --- Batch totals and per-population allocations, transcribed by hand. ---

TOTAL_CASES = 150
"""Batch totals: 125 reconciliation cases plus 25 orphan cases."""
...
"""The batch-totals table, typed out by hand. Shares 20.0 / 33.3 / 11.3 / 24.0 / 11.3%."""
...
"""The four `OPERATIONAL_EXCEPTION` subtypes plus the three others, in the batch's
subtype order.

These are `exception_subtype_recall`'s seven denominators, and their sum is
the `EXTERNAL_ACTION_REQUIRED` row above: seven subtypes divide 36 cases, six
cannot.
"""
...
The batch allocates 10 cases to each of the five auto-close families, and
each family maps to exactly one template, so a run that auto-closes every
case it should posts exactly one entry per case. Recognition entries for
review cases are not built, so no `EXTERNAL_ACTION_REQUIRED` case carries an
entry that the entry-level denominator would otherwise pick up.
"""
...
def test_every_denominator_hand_checks_against_the_batch_totals_table() -> None:
    """The checkpoint: every denominator hand-checked against the batch totals.

    Every denominator in the metrics surface, asserted against the table and
    population allocations transcribed above as literals.
    """
    _, _, report = _metrics()

    # The one denominator every `*_rate` shares.
    ...
    # The ground-truth state distribution matches the batch-totals table exactly.
    ...
    # The per-subtype denominators, seven of each, visible.
    ...
    # The macro is over seven subtypes, not six.
...
def test_ground_truth_state_counts_are_recomputed_from_labels_not_asserted_counts() -> None:
    """The checkpoint's authority: the counts above are typed by hand, the counts
...
def test_ground_truth_subtype_populations_match_the_spec_tables() -> None:
    """The four `OPERATIONAL_EXCEPTION` subtypes plus the three others, off the labels."""
    batch, _, _ = _metrics()
...
def test_denominator_convention_holds_for_every_metric() -> None:
    """The denominator convention, asserted as a property rather than read as prose.

    Metrics named `*_rate` use total cases as the denominator. Metrics named
    `*_recall` use the ground-truth population for that state. Metrics named
    `*_precision` use the population the system predicted.
    """
    _, _, report = _metrics()
    ...
    # `auto_close_precision` is the one deliberate exception:
...
def test_false_match_rate_is_not_the_complement_of_auto_match_precision() -> None:
    """The two look like complements but are not; the denominators differ."""
    _, _, report = _metrics()
...
# --- The measured surface at seed 0, against provisional targets. ---


def test_primary_safety_metrics_hold_at_seed_zero() -> None:
    """Target: `false_match_rate` 0, `auto_close_precision` >= 0.98.

    Asserted rather than eyeballed. Every figure here is provisional, but these
    two are the ones named "primary safety metric", so a regression in either
    should fail a test.
    """
    _, _, report = _metrics()
...
def test_auto_close_precision_denominator_is_the_fifty_family_entries() -> None:
    """One entry per auto-closed case: five auto-close families x 10 cases, one template each."""
    _, _, report = _metrics()
...
def test_declined_by_policy_rate_is_the_seventeen_policy_excluded_cases() -> None:
    """Expected ~11.3%, by construction rather than by performance: 5 date-error cases
    plus 12 other policy-excluded cases."""
    _, _, report = _metrics()
...
def test_declined_by_confidence_rate_has_no_ground_truth_population() -> None:
    """Every ground-truth `REVIEW_REQUIRED` case is a policy decline, so a
...
def test_abstention_rate_sits_inside_the_stated_operating_range() -> None:
    """Operating range 8-18%; ground truth is 11.3%."""
    _, _, report = _metrics()
...
def test_value_coverage_is_integer_paise_on_both_sides() -> None:
    """No float touches money. The ratio is derived; the two sides are ints."""
...
def test_report_is_identical_across_two_runs_at_the_same_seed() -> None:
    """Determinism, and the precondition for the byte-identical reproduce test."""
    _, _, first = _metrics()
    _, _, second = _metrics()
...
def test_denominators_hold_at_other_seeds(seed: int) -> None:
    """The batch allocations are the generator's contract, not a seed-0 accident.

    Seed 1 is the development batch and seed 2 is the held-out batch; the
...
def test_a_false_match_moves_every_matching_metric_by_the_exact_expected_amount() -> None:
    """The whole metrics surface reads 1.0000 on this batch at seeds 0, 1 and 2. That is
    a believable result for a deterministic pipeline graded against the generator that
    produced its inputs, but it is indistinguishable from a
...
def test_a_wrong_journal_moves_auto_close_precision_but_not_auto_close_recall() -> None:
    """The reason for an entry-level denominator: a correct match followed
...
def test_a_misclassified_subtype_moves_one_precision_and_one_recall() -> None:
    """The asymmetry, checked: assigning S to a case whose truth is T costs T's
...
def test_a_subtype_the_system_never_assigns_leaves_the_macro_over_fewer_subtypes() -> None:
    """The visible-denominator rule at the macro level: dropping every
...
def test_six_of_seven_subtypes_are_adopted_from_deterministic_triggers() -> None:
    ...
    The deterministic baseline adopts an already-fired trigger wherever the
    deterministic component fired one. Six of the seven graded subtypes have
    such a trigger, so on the baseline arm those six are correct by construction and
    only `UNMATCHED_INBOUND_CREDIT` — the one subtype that turns entirely on whether
    the free-text narration identifies a counterparty — is decided by classification
    at all. The macro is therefore not primarily a measure of the classifier on this
    arm, which is what the ablation run exists to expose and what the adversarial
    set is the independent check on.
    """
    batch, result, _ = _metrics()
    aligned = align_ground_truth(result.cases, batch.ground_truth)
    trigger_backed = {
...
def test_the_seventeen_policy_declined_cases_are_invisible_to_the_subtype_metrics() -> None:
    """A disclosed consequence of the subtype-metric definition, pinned rather than
    papered over.

    The 5 family-4 date-error and 12 other policy-excluded cases carry ground-truth
    subtypes (`MISPOSTING`, `OMISSION`) that are `ACCOUNTING_CORRECTION`-side and
    outside the classifier's eight-value vocabulary entirely, so they enter no
    per-subtype denominator on either side even though a classifier saw all 17. They
    are graded by `state_prediction_accuracy` and `declined_by_policy_rate`, and on
    the class axis by the exception-class confusion matrix.
    """
    batch, result, report = _metrics()
    aligned = align_ground_truth(result.cases, batch.ground_truth)
...
def test_macro_average_denominator_is_seven_and_numerator_is_the_defined_subtypes() -> None:
    """Seven subtypes. The numerator says how many actually entered the mean,
    so a macro computed over fewer says so on its face (the visible-denominator rule)."""
    _, _, report = _metrics()
...
def test_macro_average_never_includes_ambiguous_case() -> None:
    """`AMBIGUOUS_CASE` is an exception class, not an `OPERATIONAL_EXCEPTION` subtype —
    graded by the exception-class confusion matrix, not here."""
...
def test_auto_applied_entries_includes_replayed_entries() -> None:
    ...
    so an auto-closed case's entries arrive as `replayed_entries`. Counting
...
def test_auto_applied_entries_excludes_proposed_entries() -> None:
    """Proposed entries are unapplied by definition; the denominator here is auto-applied entries."""
    entry = _candidate("case_1", "T-01", _T01)
...
def test_entry_matching_ignores_account_name() -> None:
    """`account_code` is the key; the name is derived from it in
...
def test_orphan_case_is_worth_the_magnitude_across_its_bank_lines() -> None:
    """A `DUPLICATE_CREDIT` case spans two lines and a `REVERSAL_UNMATCHED`
...
# --- Batch alignment: the orphan case-ID join (`expected_linked_source_records`). ---
...
def test_the_two_sides_disagree_on_every_orphan_case_id() -> None:
    ...
    `case_orphan_<lowest line_id>` having never seen the generator. A
    join on `case_id` alone silently drops all 25 orphan cases — a sixth of
    the batch, and the sixth the one graded LLM slot is mostly about.
    """
    batch, result = _run()
...
def test_alignment_resolves_a_duplicate_credit_pair_to_one_case() -> None:
    """A duplicate credit and its original are one case spanning two bank
...
def test_alignment_rejects_a_ground_truth_case_spanning_two_assembled_cases() -> None:
    """The granularity rule failing. Picking either target would report a
...
def test_provenance_defaults_to_empty_and_round_trips_what_it_is_given() -> None:
    """The provenance pin is filled in by a later stage; the fields exist now so the
    JSON's shape does not change when that stage fills them, and nothing here is
...
def test_performance_metrics_are_a_separate_document_from_the_report() -> None:
    """The metrics surface lists `throughput` and `end_to_end_latency`, and the
    metrics JSON must reproduce byte-identically. Keeping wall-clock out of
    `MetricsReport` is what lets both be literally true."""
    _, _, report = _metrics()
