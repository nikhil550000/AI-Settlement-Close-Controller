...
suite (NFR-05's offline mode would be a lie if `pytest` needed a live key).
...
def test_model_id_and_base_url_are_config_not_hardcoded_at_call_sites() -> None:
    """§4.4: 'the model ID is a config parameter, not a literal, so a swap is a
    one-line change.'"""
...
def test_default_model_id_matches_the_pinned_slot_a_model() -> None:
    """Pinned to `gpt-oss-120b`, not §4.4's stated `llama-v3p3-70b-instruct` — see
    `FIREWORKS_MODEL_ID`'s docstring for why: a live-catalog check this project's own
    Fireworks account (§4.4's own "Assumption" callout requires this check) found the
