...
- **`spec.md` §8** — a revision log for defects in the *specification*. Sections lock; once locked, a change requires a numbered revision stating its issue, its change, and the alternatives rejected. Twenty-five of them, REV-01 through REV-25. The standing rule that produced most: *anything contradicting a locked section stops the session and becomes a Section 8 revision* (§6.2), and *do not invent scope — if the spec does not cover it, stop and ask* (`AGENT.md`).
...
## 1. `AUTO_CLOSED` was unreachable for all 50 cases (REV-24)

**What broke.** §1.7.4 and §4.5 put `UNIQUE(case_id, resolution_id)` on the `ledger_entry` table — a **row**-level constraint. §3.1 defines that table as one row **per leg**. §3.4 gives every one of the six correction templates two or three legs sharing one `resolution_id`. Against the DDL session 1.2 had built exactly as written:
...
Every correcting entry truncates to its first leg, fails invariant 1.7.5's `sum(debits) == sum(credits)` check, and the case is downgraded. **No case in the batch reaches `AUTO_CLOSED`** — against §3.5's allocation of 50 such cases and §2.3's ~70% build weight on that state. The headline capability of the product was arithmetically impossible.

**How it was found.** Session 4.3's checkpoint, the first time a real §3.4 template was instantiated and inserted against real SQLite. Session 1.2's checkpoint had *passed*: `tests/test_storage.py` asserts the constraint rejects a duplicate `(case_id, resolution_id)` insert, which it does. There were no multi-leg entries in existence yet to test it with. The contradiction lived between two locked sections, so no single session's tests could see it.
...
**Fix and guard.** Demonstrated first, then the session stopped rather than working around it. Logged as REV-24 (`v0.7 → v0.8`) with three candidate shapes put to the user; the chosen one widens the constraint to `UNIQUE(case_id, resolution_id, account_code)`, verified beforehand that no §3.4 template posts the same account twice within one entry. §1.7.5's entry-level check and §3.4's `resolution_id` rule are unchanged. Two tests in `tests/test_apply.py` pin both halves so neither can be lost: `test_a_three_leg_entry_posts_all_three_legs` and `test_the_widened_constraint_still_rejects_a_duplicate_leg`. REV-24 records the rejected alternatives, including the textbook `resolution` header table — rejected for adding a table the spec names nowhere.

---

## 2. Nine ground-truth labels no component could ever reach

**What broke.** The nine settlement-anchored `AMBIGUOUS_CASE` cases were built in session 2.2 as an internally-balanced contra-revenue ledger pair whose `reference` was a fresh `rfnd_*` id existing nowhere else in the batch. Measured: 18 entries, 9 distinct references, **0 resolving to anything**. That made the pair unresolvable — the intent, per §3.3's "a required piece of evidence is absent" — but it also made it **unattributable**. `ledger_entry.reference == recon_line.entity_id` is the only join between the ledger and a case, so a reference resolving to nothing belongs to no case at all. Nine of 150 cases carried `ABSTAINED` ground truth that no component, deterministic or model, could have reached: `state_prediction_accuracy` would have carried a silent ~6% ceiling and `abstention_rate` could only have been hit by accident.

**How it was found.** Session 3.3's **Next** field flagged it as a gap; session 4.1 checked the claim empirically before writing the predicate evaluator and found it was worse than a gap. Nothing was failing. The batch generated, the tests passed, and the number that would have been wrong was a metric nobody had computed yet.

**Root cause.** Not a spec defect — §3.3 and §3.5 say nothing wrong. The generator's realization of them confused *unresolvable* with *unattributable*.
...
**How it was found.** Session 3.2, immediately before writing case assembly, by reading the population the new component was about to consume. Case assembly is component 2 — fully deterministic, no LLM permitted under §4.2 — and its checkpoint is "150 cases assemble." That checkpoint could not have been hit honestly while the collision existed, and the failure would have presented as a component bug rather than a data bug.
...
**How it was found.** An adversarial pass asked why, and found the mechanism: five of the pipeline's decision boundaries are keyword lists whose coverage of the generator's own string pools is 100% hit / 0% miss **by construction** — `_RAZORPAY_MARKER`, `_REVERSAL_KEYWORDS`, `_BANK_CHARGE_KEYWORDS`, `_BANKING_BOILERPLATE_WORDS`, `_TAX_POSITION_MARKERS`, against `SETTLEMENT_PARTIES`, `REVERSAL_TEMPLATES`, `_BANK_CHARGE_NARRATIONS`, `OPAQUE_CREDIT_NARRATIONS`, `TAX_SIGNATURES`. §4.1's import guard cannot see this — a data coupling is not an import. A seed sweep cannot see it — every pool is a module constant a seed does not vary. **No test in the suite feeds the pipeline a string the generator could not have written**, which is exactly why 543 of them pass over it.

**Then it was measured, not asserted.** `tools/heldout_vocabulary.py` builds a disjoint surface vocabulary — real Indian bank-statement and settlement-adjustment wording, chosen so every replacement stays decidable by a competent reader while sharing no literal with those five lists (`RZRPAY SOFTWARE PVT LTD`, `SUSPENSE-CR`, `SUNDRY RECEIPT`, `CR CANCELLED-{ref}`). `tools/build_heldout_vocab_batch.py` rewrites a batch onto it **by template, not by search-and-replace**: each narration is matched against the generator's own `ALL_TEMPLATES` in the same precedence `narration_template` uses, its slots extracted, and the held-out template at the same index re-rendered with `{ref}` verbatim so FR-09's tier cascade sees the identical token. `ground_truth.jsonl` and `settlements.jsonl` are copied byte-for-byte, so nothing in the apparatus can move a label. Structure identical, answer key identical, surface changed. Committed at `data/heldout_vocab/`.
...
|---|---|---|
| **REV-01** — `match_rate` was defined as recall over the auto-matchable population but named as the classic finance batch-level rate. Reporting recall under a name finance reviewers read as match rate overstates the headline. | Reading locked §1.6 against its own stated definition, before any metric code existed. | Redefined with total cases as denominator; the original formula kept under the accurate name `auto_match_recall`. Both reported. REV-09 caught the identical mistake in `auto_close_rate`. |
| **REV-25** — the macro-average denominator was stated as **six** subtypes in §3.6 and §5.2's supporting clause and **seven** elsewhere. Six cannot divide the 36 cases the allocation table assigns across seven; excluding `DISPUTE_PENDING` leaves 31, so the sentence was false under either count. Moves the headline for the one graded LLM slot by up to a seventh. | Session 6.1, hand-checking every §1.6 denominator against §3.6's population table before computing anything. | Seven is normative; the two descriptive restatements corrected. `test_macro_average_denominator_is_seven_and_numerator_is_the_defined_subtypes`, `test_macro_average_never_includes_ambiguous_case`. |
| **REV-16** — `T-01` and `T-03` both fired on all 10 family-3 cases. Selection is deterministic, so a double fire had no defined resolution; if `T-01` won, the wrong entry still *balances*, so invariant 1.7.5's debit-equals-credit check passes it. | Reading v0.4's §3.4 in full before implementation. | `T-01` gains a positive conjunct making the two mutually exclusive per record, plus a §3.4 rule that the pipeline assert at instantiation that at most one predicate fires per `(case_id, entity_id)` — a double fire is a hard error, not a precedence question. |
| **`largest_gap` was decided by float noise.** Two metrics move by the same two cases in 150, so their gaps are equal — but as raw floats they differ in the sixteenth decimal (`-0.013333333333333308` vs `-0.013333333333333329`) and a plain `max` reported whichever way the subtraction rounded. A headline figure flipping on a rounding artefact. | A checkpoint test expecting `state_prediction_accuracy` failed when `max` picked `abstention_rate`. | `largest_gap_among` compares at 6 dp — finer than one case at 0.0067, coarser than the noise — and breaks ties toward §1.6's stated order. `test_a_tie_between_two_gaps_is_broken_by_order_not_by_float_noise` pins both halves, including that the raw floats really are unequal. |
| **The orphan case-ID join.** `compute_metrics` failed on its first real run with 25 unscored and 25 unlabelled cases: ground truth says `orphan_1f617519`, the assembled case says `case_orphan_bank_0442bb20`. | The error's own output. Second time this seam bit — session 5.1 records the same two ID schemes defeating a test helper. | `align_ground_truth` joins through each case's record IDs. The first fix lived in one test file; this one is production code on the graded path. Two new tests that had reproduced the bug by hand were corrected, not the code. |
| **The committed cache did not cover the pinned arm.** Session 6.2's work moved seven `ABSTAINED` cases into `EXTERNAL_ACTION_REQUIRED`, so a strict run of the pinned arm asked Slot B for narrations the cache had never seen — `CacheMissError`. | By actually trying to build the CLI's default pinned run **offline**, rather than assuming it from the arithmetic. | A real `--cache-mode refresh` pass, verified by two consecutive offline strict passes. |
| **`UnicodeEncodeError` on the one documented command.** `pipeline/cli.py`'s summary prints `§`/`×`/`—`, which this machine's default console codepage (cp1252) cannot encode. | Session 7.2, the first time *committed* code printed those to a real console. Sessions 6.3 and 7.1 had hit the identical trap in gitignored scratch scripts and correctly shrugged it off — that shrug expired the moment committed code did it. | A guarded module-level `sys.stdout/stderr.reconfigure(encoding="utf-8")` in `pipeline/cli.py`. FR-10's "the CLI is the product" means it works out of the box, not after a judge sets an environment variable. |
| **REV-23** — §2.2's ledger-entry estimate (~3,540) could not be reconciled with §3.2's locked four-leg posting. | Session 2.2's actual generator run measured it: 1,449 payment-type recon lines produced 5,774 entries, 3.98 legs/payment. Caught only then because no earlier session generated the full 125-settlement population at once. | Corrected to ~5,800 entries, ~7,600 raw records. Surfaced to the user, who chose a formal revision over a loose estimate. No case count, template or invariant affected. |
...
**Slot A degrades to the deterministic baseline.** `classify_case_llm(..., on_cache_miss="fallback")` returns `classify_case_baseline(bundle)` instead of raising, and the result carries `source = KEYWORD_BASELINE` or `DETERMINISTIC_TRIGGER` rather than `LLM_SLOT_A`. The audit trail (§1.7.3) records *how* a label was reached, so a degraded run is visibly degraded rather than silently different.

**Slot B degrades to `deterministic_narration`** — prose assembled from facts the deterministic path has already fixed, carrying `model_generated=False`. That field widened from `Literal[True]` to a **real bool** in `2ae71c1`: it had been a type that could not express the thing it was named for, which made FR-11's per-string labelling requirement unfalsifiable. It now distinguishes the two cases in the rendered report.

**The strict contract is unchanged; only the CLI opts out of it.** `CacheMode.STRICT` never constructs a network path, and a cache miss is a `CacheMissError` — §4.3's "hard error rather than a fallthrough to the API." That keeps the network off the eval path and makes NFR-05's offline mode real rather than nominal, and it remains the default for library callers (`on_cache_miss="raise"`), pinned by `test_strict_on_a_miss_raises_cache_miss_error_not_a_network_call` in both slots' test files and `test_strict_run_with_an_incomplete_cache_fails_loudly`. FR-10's CLI passes `"fallback"`, because that rule exists to protect the *measurement* and was never a reason for the product's one documented command to be brittle.

**Abstention is a designed terminal state, not a failure.** `ABSTAINED` is one of §1.3's five outcome states, with its own ground-truth population (17 of 150), its own metric, and its own §5.5 operating band (8–18%). Declining a case is a correct outcome, graded as such. This is what caught defect 6(b) from the other side: the Slot A arm reads 0.0667, *below* the band, because its eight-value output space has no "insufficient evidence in the fields provided" option, so a forced choice becomes a confident one. The band is where the metric surface caught the model converting ambiguity into confident exceptions — the error direction §1.3 ranks worst.

**Policy refusal is separated from low confidence.** REV-02 split `declined_by_policy_rate` from `declined_by_confidence_rate` so that deliberate scope discipline (tax positions; date-only reclassification across a period boundary) is not reported as weak detection. At the pinned run: 17 policy, 0 confidence.

**A pre-committed cut order.** `AGENT.md` §2.0 fixes what gets dropped if the build falls behind, mechanically rather than improvised: (1) drop family 5, removing 10 cases and templates `T-05`/`T-06`; (2) drop the third bank format profile; (3) reduce the reference batch to 100 cases — last resort, cheapest to cut and most expensive to lose, since every metric denominator shrinks with it. Tags `phase-1` … `phase-7` are the recovery points that order assumes. Nothing was cut in the end, which is the only reason this reads as unused rather than as the reason the build shipped.

**The stop-and-ask rule.** *Do not invent scope; if the spec does not cover it, stop and ask.* Incidents 1, 2, 3 and REV-23 all reached the user as a question with options rather than as a unilateral workaround. Twenty-five numbered revisions is the cost of that rule; `AUTO_CLOSED` being reachable is what it bought.
