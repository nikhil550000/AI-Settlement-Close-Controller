...
There are five model-touching surfaces, each separately switchable and separately measured. All five sit on the same side of invariant 1.7.2's line: they may classify, read prose, select among candidates a deterministic rule already validated, or propose a *shape*; none may originate an account, an amount, or a narration on the automated path.

**Semantics — five routing reads. Keyword arm and LLM arm. Graded by the whole §1.6 surface.** `pipeline/semantics.py`. Does this narration name a counterparty; is this credit the gateway; is this a reversal; is this a bank charge; is this a tax position. (The sixth read, contested-credit resolution, is separated out below because it is the only one that can misroute money.) Each was previously a literal-substring test living inside the component that asked it, and each separated the generator's corresponding string pool with 100% hit and 0% miss — which is the reason the reference batch grades 1.0000 with no model involved. `KeywordSemantics` is that logic relocated, not rewritten, and is the default at every call site, so the committed run reproduces byte-identically. `LlmSemantics` answers the same five under constrained decoding through the SHA-keyed cache. The interface returns only a `bool` or a counterparty name lifted verbatim out of text the bank wrote — verified as a substring before it is trusted — so its answers route cases and never price one.
